...
# Hàm binning cho RUL (để giảm độ phân giải của RUL)
def bin_rul(rul, bin_size=0.05):
    """
    Bin the RUL values into specified intervals. For example, if bin_size=0.05,
    values between 0.95-1.00 will be binned as 1.0, values between 0.90-0.95 will be binned as 0.95, etc.
    """
    return np.floor(rul / bin_size) * bin_size

# Hàm tính toán RUL và gán nhãn theo nhóm bin
def calculate_rul_with_bins(fpt_index, failure_time, time_series_length, bin_size=0.05):
    """
    Calculate RUL and bin the values into categories.
    """
    rul = calculate_rul(fpt_index, failure_time, time_series_length)
    binned_rul = np.array([bin_rul(r) for r in rul])
    return binned_rul
...
def label_rul_for_all_files(file_list, bearing_failure_time, bin_size=0.05):
    ...
    for file in file_list:
        ...
        output_file_path = file.replace('.csv', '_RUL_labeled_2.csv')
...
file_list = [
             "D:\\XJTU-SY_Bearing_Datasets\\37.5Hz11kN_denoise_by_LSTM_Autoencoder\\Bearing2_5.csv"]  # Cập nhật đường dẫn file của bạn
bearing_failure_time = {
    'Bearing1_1': 123,  # 2h 3m = 123 phút
    'Bearing1_2': 161,
    'Bearing1_3': 158,
    'Bearing1_4': 122,
    'Bearing1_5': 52,
