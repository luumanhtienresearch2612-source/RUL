...
"""
XGBoost_Regressor
This script trains, saves, and evaluates XGBoost models for 
Remaining Useful Life (RUL) prediction across various denoised signals.
"""
...
for col in signal_columns:
    ...
    model_save_path = os.path.join(model_save_dir, f"RUL_model_XGBoost_{col}.json")
    ...
    plt.title(f"RUL Prediction vs Actual RUL - XGBoost ({col})")
    plt.xlabel("Time (s)")
    plt.ylabel("RUL")
    plt.legend()

    # Save Chart
    chart_save_path = os.path.join(chart_save_dir, f"RUL_model_XGBoost_{col}.png")
